...
export function EditAccountForm({ account, onSubmit, onClose }: EditAccountFormProps) {
  ...
  const [selectedRegions, setSelectedRegions] = useState<string[]>(
    account.regions && account.regions.length > 0 ? account.regions : ["ap-south-1"],
  );
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!name || selectedRegions.length === 0) {
      setError("Name and at least one region are required.");
      return;
    }
    setLoading(true);
    setError(null);
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40">
      <div className="bg-white rounded-xl shadow-2xl w-full max-w-lg mx-4 animate-fade-in">
        <div className="flex items-center justify-between px-6 py-4 border-b border-slate-200">
          <div>
            <h2 className="text-lg font-semibold text-slate-900">Edit AWS Account</h2>
        ...
        <form onSubmit={handleSubmit} className="px-6 py-5 space-y-4">
          ...
          <div>
            ...
            <input
              ...
              placeholder="arn:aws:iam::123456789012:role/CSPMScanRole"
          ...
          <div>
            <label className="block text-sm font-medium text-slate-700 mb-1">Active Regions</label>
